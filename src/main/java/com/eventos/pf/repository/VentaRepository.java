...
public interface VentaRepository extends JpaRepository<Venta, Long>, JpaSpecificationExecutor<Venta> {
    ...
    @Query(
        value = "select venta from Venta venta left join fetch venta.usuario left join fetch venta.evento",
        countQuery = "select count(venta) from Venta venta"
    )
    Page<Venta> findAllWithToOneRelationships(Pageable pageable);

    @Query("select venta from Venta venta left join fetch venta.usuario left join fetch venta.evento")
    List<Venta> findAllWithToOneRelationships();

    @Query("select venta from Venta venta left join fetch venta.usuario left join fetch venta.evento where venta.id =:id")
